...
class ForkManager
{
     ...
     * @param callback        $preCallback  pre callback
     * @param callback        $postCallback post callback
     *
     * @return int the internal identifier
     */
    public function addCommand(Command $command, InputInterface $input, OutputInterface $output, $preCallback = null, $postCallback = null)
    {
        $this->queue[] = array($command, $input, $output, $preCallback, $postCallback);
    ...
    public function runSilence(OutputInterface $output)
    {
        ...
        foreach ($this->queue as $key => $command) {
            list($command, $input, $output, $preCallback, $postCallback) = $command;
